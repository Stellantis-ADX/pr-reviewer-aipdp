...
def run():
    try:
        options = Options(
            ...
            databricks_base_url=get_input_default(
                ACTION_INPUTS, key="databricks_base_url"
            ),
            databricks_token=get_input_default(
                ACTION_INPUTS, key="databricks_token"
            ),
            light_model_name_claude=get_input_default(
                ACTION_INPUTS, key="light_model_name_claude"
            ),
            heavy_model_name_claude=get_input_default(
                ACTION_INPUTS, key="heavy_model_name_claude"
            ),
        )
        ...
        # Using Databricks-hosted Claude models

        try:
            # Check if we should use Claude bots (Databricks)
            if options.databricks_token and options.databricks_base_url:
                # Use Claude bots from Databricks
                light_bot = ClaudeBot(
                    options,
                    ClaudeOptions(
                        options.light_model_name_claude or "claude-sonnet-4-5",
                        options.light_token_limits_claude
                    ),
            ...
            warning(
                f"Skipped: failed to create summary bot: {e}, "
        ...
        try:
            ...
            if options.databricks_token and options.databricks_base_url:
                ...
                heavy_bot = ClaudeBot(
                    options,
                    ClaudeOptions(
                        options.heavy_model_name_claude or "claude-opus-4-6",
                        options.heavy_token_limits_claude
                    ),
                    api_key=options.databricks_token,
                    base_url=options.databricks_base_url,
                )
            ...
            warning(
                f"Skipped: failed to create review bot:"
